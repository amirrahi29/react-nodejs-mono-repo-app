...
async function loadAppSecrets() {
    ...
    if (!username && !password) {
      console.log(
        JSON.stringify({
          ...
          hint:
            "Set AZURE_KEY_VAULT_URL (Helm/GitHub secret) or APP_USERNAME/APP_PASSWORD (credentialsSecret / .env)",
        })
      );
    } else {
  ...
  try {
    ...
    throw new Error(
      `Key Vault failed (${code}): ${err.message}. Check URL, names (${userKey}, ${passKey}), AZURE_CLIENT_ID + Workload Identity, and Key Vault RBAC.`
    );
  }
    ...
    const which =
      username === "" && password === ""
        ...
        : username === ""
          ...
          : `"${passKey}"`;
    throw new Error(
      `Key Vault returned empty value for ${which}. Check secret names and enabled versions.`
    );
  }
